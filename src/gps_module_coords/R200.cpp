        ...
        switch(_buffer[R200_CommandPos]){
          ...
          case CMD_SinglePollInstruction:
            ...
            for(uint8_t i = 0; i < 4; i++) {
              if(memcmp(uid[i], &_buffer[24 * i + 9], 12) != 0) {
                memcpy(uid[i], &_buffer[24 * i + 9], 12);
                #ifdef DEBUG
...
void R200::dumpUIDToSerial(){
  Serial.println("Dumping UIDs...");
  for (uint8_t i=0; i < 4; i++){
    Serial.print("0x");
    for(uint8_t j = 0; j < 12; j++) {
      Serial.print(uid[i][j] < 0x10 ? "0" : "");
      Serial.print(uid[i][j], HEX);
    }
    Serial.println();
  }
  // Serial.println(". Done.");
...
bool R200::receiveData(unsigned long timeOut){
  ...
  while ((millis() - startTime) < timeOut) {
    while (_serial->available()) {
      ...
      if(bytesReceived > RX_BUFFER_LENGTH - 1) {
        ...
        flush();
        return true; // NEW: was false initially
      }
      else {
  ...
  // NEW: Checks incorrectly - need to check all frames!
