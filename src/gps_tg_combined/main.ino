...
//--Network credentials: CHANGE WIFI!!!
#define WIFI_SSID "NU"
#define WIFI_PASSWORD "1234512345"

//--Firebase project API Key:
#define API_KEY "AIzaSyBtiKdiBklG4HqFCUKmd_KQorXfqUjwTcw"
//--RTDB URL:
#define DATABASE_URL "https://next-rfid-ddd13-default-rtdb.europe-west1.firebasedatabase.app/"

//!!!!! Define Telegram-related variables:
//--Bot token:
#define BOTtoken "6994546277:AAGKDHT1fYMDGpkhR0y47l4UedTla9cwc0w"
//--TG ID:
#define CHAT_ID "716289917"
...
unsigned long gpsModuleResetTime = 0;
...
//!!!!! Function for getting GPS coordinates:
bool get_gps() {
  //Serial.println("Entered GPS function");
  if (Serial1.available() > 0) {
    //Serial.println("Serial1 available");
    if (gps.encode(Serial1.read())) {
      //Serial.println("Encoding success");
...
//!!!!! Function for handling location messages:
...
//!!!!! Function for avoiding duplicate sends within the same run:
...
//!!!!! Function for resetting the sent array:
void reset_sent() {
  for (int i = 0; i < 20; i++) {
    sent[i] = "0";
  }
...
void loop() {
  ...
  //!!!!! Try to get coordinates from the GPS module:
  gpsModuleResetTime = millis();
  while (latitude == "" && longitude == "" && millis() - gpsModuleResetTime <= 20000) {
    if (get_gps()) {
      lastResetTime = millis();
    }
  }
  ...
  if (latitude == "" && longitude == "") {
    Serial.println("Please send your location to the Telegram bot!");
    bot.sendMessage(CHAT_ID, "Please send your current location!", "Markdown");
  }
